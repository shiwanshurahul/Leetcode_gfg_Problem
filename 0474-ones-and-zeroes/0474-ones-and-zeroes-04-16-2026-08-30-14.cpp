...
int solve(vector<string>& strs, int m, int n, int i){
    ...
    int cnt_0 = 0;  //keep track -> har recursiv call ka apn
    int cnt_1 = 0;   //chah toh m-- v kr skte us case me m_=m rakh lo for skip

    for(int j=0;j<strs[i].length();j++){  //ith idx wal pe
        if(strs[i][j] == '0')
           cnt_0++;
        else
            cnt_1++;    
