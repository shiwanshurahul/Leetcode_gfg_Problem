...
int solve(int i,int j, vector<vector<int>> &matrix,int n, int m, vector<vector<int>> & dp){
    ...
    return dp[i][j] = 1+ min(right, min(down, diagonal)); //har 1 val wale index ka ans bhej rha   
}
...
int top_down_tabulation(vector<vector<int>> &matrix, int n,int m){
    ...
    int countSquares(vector<vector<int>>& matrix) {
        ...
        // for(int i=0;i<n;i++){
        //     for(int j=0;j<m;j++){
        //         if(matrix[i][j] == 1)  //har 1 pe find ans -> idhar v teeno recursive functi call ks skte
