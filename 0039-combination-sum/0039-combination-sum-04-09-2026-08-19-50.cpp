class Solution {
public: //backtrackin -> pick krke explore fir skip krke explore
    ...
    void solve(vector<int> &candidates, int target, int i){
        ...
        solve(candidates,target-candidates[i], i);  //firs choose kr skte
        v.pop_back(); //after recursive call
        solve(candidates,target,i+1);  //skip -> dusre no leng
          
    }
    vector<vector<int>> combinationSum(vector<int>& candidates, int target) {
        //all possible combination/permutaio -> backtrackin
        solve(candidates, target, 0);
        return ans;
    }

    //    int pick = solve(candidates, target-candidates[i],i);
    //     int skip = solve(candidates, target,i+1);
    //     return pick + skip; -> ye no of combinstn dega but exac elem ni
