class Solution {
public: //djikstr ke liye edges wt chahiye
typedef pair<int,int> p;
    int networkDelayTime(vector<vector<int>>& times, int n, int k) {
        ...
        // e.g. for 2 -> {1,1}, {3,1}
        // idhar vector<pair<int,int>> isliye hota bcz 
        // there can be multiple adj nodes

        for(auto it: times){
            int u = it[0];
            int v = it[1];
            int w = it[2];
            adj[u].push_back({v, w});  //directd grp
        }

        //dijkstr -> nbr ke via min dist ki kosis
        //dijkstr is single src to each node ka min duratn
        ...
        while(!pq.empty()){   //heap ka
            ...
            int node = pq.top().second; //us node tak ka distanc from 0/src 
            pq.pop();
            for(auto it: adj[node]){  //u -> {v1, w1}, {v2,wt2} = pair = it.first, it.second
                int adjNode  = it.first; //adj list ais hai-> {node,node_wt}
