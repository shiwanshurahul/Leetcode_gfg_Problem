class Solution {
public:
//expres base case in i and j
//explor all possible solutions
//return min, max, sum ..
...
int solve(vector<vector<int>> &matrix, int i, int j, int n, int m, vector<vector<int>> &dp){
    ...
    if(j<0 || j> m-1 || i<0 || i> n-1 )
     ...
     if(i == n-1 )  //las row
        return matrix[i][j];   //add is idx ki value to your sum
...
int tabulation(vector<vector<int>>& matrix){
        //bottom -up ko opposit dirn me likht as recusio
        int n = matrix.size();
        int m = matrix[0].size();
        
       vector<vector<int>>dp(n, vector<int>(n,1e9));

        for(int i=0;i<n;i++){    //first row me val matrix se populate kr
            dp[0][i] = matrix[0][i];
        }
    ...
    int minFallingPathSum(vector<vector<int>>& matrix) {
        ...
        // for(int col=0;col<m;col++){  //firs row me sare col se chk
