...
int solve(vector<int> &prices, int k, int i, int prev, vector<vector<vector<int>>> &dp){
    if(i>= prices.size() || k == 0)  //0 hote retur k<=0 de skte
        return 0;
    ...
    if(prices[i] > prices[prev]){   // && k!=0 laga skte but base case me handle kr rhe
        ...
        not_sell = solve(prices,k,i+1,prev, dp);  // dp choice -> is profit pe ni bechna
         ...
         not_sell = solve(prices,k,i+1,i, dp);  //2no badh do
    }
...
int solve1(vector<int> &prices, int i, bool buy, int k, vector<vector<vector<int>>> &dp){
    ...
    if(buy){  //allowd 
        ...
        int skip = solve1(prices,i+1,true,k, dp);  //buy kr skt
        return dp[i][buy][k] = max(buy_kiya,skip);
    }   
    else{ //buy is not allowd
    ...
    int maxProfit(vector<int>& prices) {
        ...
        // return solve(prices, 2, 1, 0, dp); //pehle din buy -> lis jaisa
        //memory level exceeded

        //m-2: memory optimized ->har idx pe 2 option -> buy, sell = dono explore with dp
         vector<vector<vector<int>>> dp(prices.size()+1, vector<vector<int>>(2,vector<int>(3,-1)));
         return solve1(prices,0,true,2,dp); //buy is true for 1st idx
