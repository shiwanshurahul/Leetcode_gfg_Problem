...
int solve(int i, int j, vector<int>&piles){
    //Alice can pik either i or j
    if(i>j)
        return 0;
    if(dp[i][j] !=-1)
        return dp[i][j];

    int ith_pick = piles[i] + solve(i+1,j, piles);  //beginin
    int jth_pick = piles[j] + solve(i,j-1, piles);  //peeche s
    ...
    bool stoneGame(vector<int>& piles) {
        ...
        //har pont pe 2 case -> beginin se pick or end se = explor both
        int n = piles.size();
