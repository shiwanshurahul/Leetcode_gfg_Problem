...
int solve(string s, int i, vector<int> &dp){
    ...
    if(mp.find(temp) == mp.end() ) //0 wala case handle ->sedha s[i] krte but mp me string 
        return 0;     //06 -> 0th idx se seedha 0 return to main, fir no execution

    int single_len = 0;
    int two_len = 0;

    single_len = solve(s,i+1, dp);  //i+1 wala check kro ->ek char krk i==n tak pahunch fir backtrak krke 2 len wale check
